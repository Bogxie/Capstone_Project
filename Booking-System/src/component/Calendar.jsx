    ...
    const getDayClasses = (bookedServiceList, isToday, isPast, isBlocked) => {
    ...
    export const Calendar = ({ bookings, addBooking, disableServices, blackoutDates = [] }) => {
        ...
        // FIX: Sinali ang isBlocked sa parameters para gumana ang restriction sa click
        const handleDayClick = (dayNumber, isPast, isFullyBooked, bookedServices = [], isBlocked) => {
        ...
        const renderDays = () => {
            ...
            for (let dayNumber = 1; dayNumber <= daysInMonth; dayNumber++) {
                ...
                // FIX: Dito lang dapat icheck ang blackout list para sa araw na ito
                ...
                cells.push(
                    <div
                        key={dayNumber}
                        onClick={() => handleDayClick(dayNumber, isPast, isFullyBooked, bookedServices, isBlocked)}
                        className={getDayClasses(bookedServiceList, isToday, isPast, isBlocked)}
                        style={!isBlocked ? (stripGradient ? { background: stripGradient } : undefined) : undefined}
                        title={isBlocked ? "Blocked by Admin" : bookedServiceList.length > 0 ? `Booked: ${bookedServiceList.join(", ")}` : undefined}
                    >
                        <span>{dayNumber}</span>
                        {isBlocked && (
